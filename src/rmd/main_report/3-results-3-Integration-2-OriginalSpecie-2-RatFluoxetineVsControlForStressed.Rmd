#### Rat Stress Alone VS Stress Fluoxetine in Dentate Gyrus{#resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetine}

This section corresponds to the second meta-analysis requested by El Chérif.

```{r resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineSettings}
fig_width <- 16
fig_height <- 26
n_cols <- 6
```

```{r resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineAlphabet, dependson="resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "Gene.name",
  "baseMean",
  "log2FoldChange",
...
The xlsx table with all genes is available [here](xlsx/resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineExportAllGenes.xlsx)

```{r resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineExportAllGenes}
# Export to xlsx
...
```{r resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineConsensus, dependson="resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineConsensusSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (p-value < 0.05 and p-value rank < 301). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationOriginalSpecieRatStressAloneVsStressFluoxetineConsensus.xlsx)"}
