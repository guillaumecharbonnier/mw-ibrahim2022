#### Rat+Mouse Fluoxetine VS Control for Stressed in Dentate Gyrus{#resultsIntegrationRatMouseFluoxetineVsControlForStressed}

This section corresponds to the first+second meta-analysis requested by El Chérif.

```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedSettings}
fig_width <- 36
fig_height <- 32
n_cols <- 17
```

```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedAlphabet, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "hugo_symbol",
  "ensembl_gene_id",
  "baseMean",
...
```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedConsensus, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
...
```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedConsensusPadj0p0001, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.0001). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationRatMouseFluoxetineVsControlForStressedConsensusPadj0p0001.xlsx)"}
