#### Rat+Mouse Fluoxetine VS Control for Stressed and no stressed in Dentate Gyrus{#resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressed}

This section corresponds to the 4 meta-analysis requested by El Chérif.

```{r resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedSettings}
fig_width <- 76
fig_height <- 32
n_cols <- 21
```

```{r resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedAlphabet, dependson="resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "Gene.name",
  "baseMean",
  "log2FoldChange",
...
d <- data.table(
    ...
    data.frame(dea_res$microarray_v2$GSE6476$FluoxetineVsControl[, cols_to_keep]),
    data.frame(dea_res$rnaseq$SRP057486$FluoxetineVsVehicleForBrainCA1NonShocked[, cols_to_keep]),
    data.frame(dea_res$microarray_v2$GSE118669$FluoxetineVsVehicle[, cols_to_keep]),
    data.frame(dea_res$microarray_v2$GSE54307$FluoxetineVsVehicle[, cols_to_keep]),
    data.frame(dea_res$microarray_v2$GSE84185$FluoxetineVsControlForNoStressedInDentateGyrus[, cols_to_keep]),
    ...
    data.frame(dea_res$microarray_v2$GSE42940$FluoxetineVsControl[, cols_to_keep])
  )
)
d <- d[!is.na(d$pvalue), ]
...
The xlsx table with all genes is available [here](xlsx/resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedExportAllGenes.xlsx)

```{r resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedExportAllGenes}
# Export to xlsx
...
```{r resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedConsensus, dependson="resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedConsensusSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (p-value < 0.05 and p-value rank < 301). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationOriginalSpecieRatMouseFluoxetineVsControlForStressedAndNoStressedConsensus.xlsx)"}
