#### Fluoxetine VS Vehicle {#resultsMicroarrayGSE54307FluoxetineVsVehicle}

##### Default settings {#resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettings}

```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettings}
gset <- getGEO(
  "GSE54307",
  destdir = geo_dir,
  GSEMatrix = TRUE,
...
fData(gset) <- cbind(
  fData(gset),
  annotations$AFFY_Mouse430A_2[
    match(
      fData(gset)$ID,
      annotations$AFFY_Mouse430A_2$probe
    )
  ]
)
...
gs <- factor(sml)
groups <- make.names(c("Fluoxetine","Vehicle","Other"))
levels(gs) <- groups
gset$group <- gs
...
par(mar = c(7, 4, 2, 1))
title <- paste("GSE54307", "/", annotation(gset), sep = "")
boxplot(ex[, ord], boxwex = 0.6, notch = T, main = title, outline = FALSE, las = 2, col = gs[ord])
legend("topleft", groups, fill = palette(), bty = "n")

# expression value distribution
par(mar = c(4, 4, 2, 1))
title <- paste("GSE54307", "/", annotation(gset), " value distribution", sep = "")
...
plotSA(fit2, main = "Mean variance trend, GSE54307")
```

```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsToIntegrationOriginalSpecie}
...
```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsToIntegration}
tT2 <- tT2[
  !tT2$Gene.symbol %in% "",
]
...
```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsHeatmapSettings}
fig_height <- 30
fig_width <- 5
n_top_genes <- 200
...
```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsHeatmapRaw, dependson="resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsHeatmapSettings", fig.height=fig_height, fig.width=fig_width}
set.seed(123)
ht = Heatmap(
  exprs(gsubset),
...
```{r resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsHeatmapZscore, dependson="resultsMicroarrayGSE54307FluoxetineVsVehicleDefaultSettingsHeatmapSettings", fig.height=fig_height, fig.width=fig_width}
set.seed(123)
ht = Heatmap(
