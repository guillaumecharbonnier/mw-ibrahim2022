#### Rat Stress Alone VS Stress Fluoxetine in Dentate Gyrus{#resultsIntegrationRatStressAloneVsStressFluoxetine}

This section corresponds to the second meta-analysis requested by El Chérif.

```{r resultsIntegrationRatStressAloneVsStressFluoxetineSettings}
fig_width <- 16
fig_height <- 26
n_cols <- 6
```

```{r resultsIntegrationRatStressAloneVsStressFluoxetineAlphabet, dependson="resultsIntegrationRatStressAloneVsStressFluoxetineSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "hugo_symbol",
  "ensembl_gene_id",
  "baseMean",
...
```{r resultsIntegrationRatStressAloneVsStressFluoxetineConsensus, dependson="resultsIntegrationRatStressAloneVsStressFluoxetineSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
...
```{r resultsIntegrationRatStressAloneVsStressFluoxetineSettings2}
fig_width <- 7
fig_height <- 7
n_cols <- 1
```

```{r resultsIntegrationRatStressAloneVsStressFluoxetineConsensusPadj0p0001, dependson="resultsIntegrationRatStressAloneVsStressFluoxetineSettings2", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.0001). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationRatStressAloneVsStressFluoxetineConsensusPadj0p0001.xlsx)"}
