#### Rat Fluoxetine VS Control for NoStressed  in Dentate Gyrus{#resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressed}

This section corresponds to the fourth meta-analysis requested by El Chérif.

```{r resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressedSettings}
fig_width <- 9
fig_height <- 9
n_cols <- 5
```

```{r resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressedAlphabet, dependson="resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "Gene.name",
  "baseMean",
  "log2FoldChange",
...
The xlsx table with all genes is available [here](xlsx/resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressedExportAllGenes.xlsx)

```{r resultsIntegrationOriginalSpecieRatFluoxetineVsControlNoStressedExportAllGenes}
# Export to xlsx
