### To closest human ortholog {#resultsIntegrationToHumanOrtholog}

#### Mouse Fluoxetine VS Control for Stressed in Dentate Gyrus{#resultsIntegrationMouseFluoxetineVsControlForStressed}

This section corresponds to the first meta-analysis requested by El Chérif.

```{r resultsIntegrationMouseFluoxetineVsControlForStressedSettings}
fig_width <- 34
fig_height <- 32
n_cols <- 15
```

```{r resultsIntegrationMouseFluoxetineVsControlForStressedAlphabet, dependson="resultsIntegrationMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "hugo_symbol",
  "ensembl_gene_id",
  "baseMean",
...
d <- rbind(
  data.frame(dea_res$microarray$GSE43261$FluoxetineResponderVsControl[, cols_to_keep]),
  data.frame(dea_res$microarray$GSE84185$StressedFluoxetineVsStressedAloneInDentateGyrus[, cols_to_keep]),
  data.frame(dea_res$microarray$GSE84185$FluoxetineVsControlForStressedResponderInDentateGyrus[, cols_to_keep]),
  data.frame(dea_res$microarray$Surget2009$FluoxetineVsSalineForStressedInDentateGyrus[, cols_to_keep]),
...
```{r resultsIntegrationMouseFluoxetineVsControlForStressedConsensus, dependson="resultsIntegrationMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
...
```{r resultsIntegrationMouseFluoxetineVsControlForStressedConsensusPadj0p0001, dependson="resultsIntegrationMouseFluoxetineVsControlForStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.0001). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationMouseFluoxetineVsControlForStressedConsensusPadj0p0001.xlsx)"}
