#### Rat+Mouse Fluoxetine VS Control for Stressed and no stressed in Dentate Gyrus{#resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressed}

This section corresponds to the 4 meta-analysis requested by El Chérif.

```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedSettings}
fig_width <- 48
fig_height <- 32
n_cols <- 20
```

```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedAlphabet, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Alphabetically-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
cols_to_keep <- c(
  "hugo_symbol",
  "ensembl_gene_id",
  "baseMean",
...
d <- rbind(
  ...
  data.frame(dea_res$microarray$GSE56028$FluoxetineVsUntreatedForStressed[, cols_to_keep]),
  ...
  data.frame(dea_res$microarray$GSE42940$FluoxetineVsControl[, cols_to_keep])
)
...
```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedConsensus, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.05). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05)."}
...
```{r resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedConsensusPadj0p0001, dependson="resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedSettings", fig.height=fig_height, fig.width=fig_width, fig.cap="Consensus-sorted genes significant in at least one comparison (padj < 0.0001). + and – signs highlight respectively upregulated and downregulated genes (|logFoldChange| > 0.1 and raw pvalue < 0.05). Underlying table is available [here](xlsx/resultsIntegrationRatMouseFluoxetineVsControlForStressedAndNoStressedConsensusPadj0p0001.xlsx)"}
