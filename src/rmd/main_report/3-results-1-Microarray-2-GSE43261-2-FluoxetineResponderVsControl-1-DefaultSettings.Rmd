#### Fluoxetine Responder VS Control for Stressed mice{#resultsMicroarrayGSE43261FluoxetineResponderVsControl}

##### Default settings {#resultsMicroarrayGSE43261FluoxetineResponderVsControlDefaultSettings}

```{r resultsMicroarrayGSE43261FluoxetineResponderVsControlDefaultSettings}
...
gset <- gset[
  ,
  gset[["response:ch1"]] %in% c(
    "control",
    "Responder"
  )
]

treatment <- factor(make.names(gset[["treatment:ch1"]]))
tissue <- factor(make.names(gset[["tissue:ch1"]]))
...
design <- model.matrix(~ 0 + treatment + tissue, gset)

fit <- lmFit(gset, design) # fit linear model

# set up contrasts of interest and recalculate model coefficients
contrast_matrix <- makeContrasts(
  contrasts = "treatmentFluoxetine-treatmentuntreated",
  levels = design
)
...
# ))
# par(mar = c(7, 4, 2, 1))
# title <- paste("GSE43261", "/", annotation(gset), sep = "")
...
# par(mar = c(4, 4, 2, 1))
# title <- paste("GSE43261", "/", annotation(gset), " value distribution", sep = "")
...
plotSA(
  fit2,
  main = "Mean variance trend, GSE43261"
)
```

```{r resultsMicroarrayGSE43261FluoxetineResponderVsControlDefaultSettingsToIntegrationOriginalSpecie}
...
project <- "GSE43261"
comparison <- "FluoxetineResponderVsControl"
tT2$project <- project
...
```{r resultsMicroarrayGSE43261FluoxetineResponderVsControlDefaultSettingsToIntegration}
